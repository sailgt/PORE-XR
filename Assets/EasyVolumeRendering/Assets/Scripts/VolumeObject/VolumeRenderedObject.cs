...
namespace UnityVolumeRendering
{
    ...
    public class VolumeRenderedObject : MonoBehaviour
    {
        ...
        {
            // Explicit cleanup required when domain reload is disabled — Unity does not auto-destroy
            // UnityEngine.Objects (Material, Texture, ScriptableObject) created at runtime with new/CreateInstance.
            if (meshRenderer != null && meshRenderer.sharedMaterial != null)
                Destroy(meshRenderer.sharedMaterial);
            if (transferFunction != null)
                Destroy(transferFunction);
            if (transferFunction2D != null)
                Destroy(transferFunction2D);
            if (dataset != null)
                Destroy(dataset);
        }
